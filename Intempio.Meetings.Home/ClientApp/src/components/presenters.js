...
export default class Presenters extends Component {
    ...
    constructor(props) {
        super(props);
        this.state = { presenters: [], loading: true, email: '' };
    }

    openInNewTab = (url) => {
        ActivityLog.getStringValue(this.state.email, "Presenter-Clicked", url);
        const newWindow = window.open(url, '_blank', 'noopener,noreferrer')
        if (newWindow) newWindow.opener = null
    }

    openInNewTabv2 = (url, description) => {
        ActivityLog.getStringValue(this.state.email, "Presenter-Clicked", url);
    ...
    async getPresenters() {
        ...
        const finalresult = await response.json().then(async (resonse) => {
            ...
            if (item && item.value) {
                ...
                var sortedp = item.value.sort((a, b) => a.fields.OrderBy - b.fields.OrderBy);
                this.setState({ presenters: sortedp });
                return true;
    ...
    render() {
        ...
        return (
            ...
            <>
                ...
                <div id={this.props.category} class="presenters" style={{ display: !this.state.loading ? "" : "none" }}>
                            ...
                            this.state.presenters && this.state.presenters.map((item, i) => {
                                ...
                                return (
                                    ...
                                    <div class="presenter"  >
                                        ...
                                        <div class="name bold" onClick={() => this.openInNewTabv2(item.fields.OtherUrl ? item.fields.OtherUrl.Url : '#', item.fields.Title)}>{name}</div>
                                        {title && <div class="name italic" onClick={() => this.openInNewTabv2(item.fields.OtherUrl ? item.fields.OtherUrl.Url : '#', item.fields.Title)}>{title}</div>}
