    ...
    async getSettingsv2() {
        ...
        const finalresult = await response.json().then(async (resonse) => {
            ...
            if (item) {
                ...
                this.setState({ loading: false, generalMsgKey: item.intempioSettings.generalChatName, helpMsgKey: item.intempioSettings.helpChatName, currentChatKey: item.intempioSettings.generalChatName, customChatRooms: item.intempioSettings.customChatGroups.split(','), chatFeature: chat });
                        ...
                        {this.state.chatFeature &&
                            <>
                                <div id="messages" class="msgs">
                                    ...
                                    <svg width="35" height="35" viewBox="0 0 24 22" fill="none" xmlns="http://www.w3.org/2000/svg">
